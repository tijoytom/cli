...
validate_env_variables(){
    ...
    if [[ -z "$DOTNETCLI_BUILD_VERSION" ]]; then
        warning "DOTNETCLI_BUILD_VERSION environment variable not set"
        ret=1
    fi
...
upload_binaries_to_blob_storage(){
    ...
    local blob="$CHANNEL/Binaries/$DOTNETCLI_BUILD_VERSION/$filename"
    ...
    local indexContent="Binaries/$DOTNETCLI_BUILD_VERSION/$filename"
...
upload_installers_to_blob_storage(){
    ...
    local blob="$CHANNEL/Installers/$DOTNETCLI_BUILD_VERSION/$filename"
