            ...
            const char** managedAssemblyArgv)
{
    ...
    // Check if there is a TPA list file on disk
    if(HasTpaListFile(clrFilesAbsolutePath))
    {
        AddFilesFromTpaListFile(clrFilesAbsolutePath);
    }
    else
    {
    ...
    if (coreclrLib != nullptr)
    {
        ...
        else
        {
            ...
            {
                Trace("executeAssembly(0x%p, %d, %d, %s, %s, %s, 0x%p)", hostHandle, domainId, managedAssemblyArgc, managedAssemblyArgv[0], managedAssemblyArgv[1], managedAssemblyAbsolutePath, &exitCode);
                st = executeAssembly(
                        hostHandle,
                        domainId,
                        managedAssemblyArgc,
                        ...
                        (unsigned int*)&exitCode);
                Trace("Executed Managed Entry Point");
                ...
                Trace("Managed Entry Point returned. Exit code: %d", exitCode);
