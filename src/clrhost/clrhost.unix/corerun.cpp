...
void DisplayUsage()
{
    fprintf(
        ...
        "-t, --trace     write trace information to stderr\n");
}
...
int corerun(const int argc, const char* argv[])
{
    ...
    Trace("Using CLR Path: %s", clrFilesPath);
    Trace("Using Managed Assembly: %s", managedAssemblyPath);
    ...
    Trace("Launching managed assembly");
    int exitCode = ExecuteManagedAssembly(
                            ...
                            managedAssemblyArgc,
                            managedAssemblyArgv);
    Trace("Managed exit code: %d", exitCode);
